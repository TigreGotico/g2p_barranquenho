...
surface. `phonemize`, working one word at a time, cannot show them.
...
The same base vowel maps to a different IPA symbol depending on its accent and
position. Spell the accents, and the open/closed/nasal distinctions come
through:
...
A vowel directly before a syllable-final `m` or `n` nasalises and swallows that
...
Barranquenho writes `b` for `v` (betacism), so `vinho` becomes `ˈbiɲu`. `g`
before a front vowel is `[ʒ]` (`gelo` becomes `ˈʒɛlu`).
...
adjacent characters. Match it as a substring of the joined output, not as a
single list item.

## The `x` letter

Word-medial `x` has no single rule in Portuguese-based spelling, so the spec
picks `ʃ`, `s` or `z` from context:
...
- **`phonemize` takes one word. `transcribe` takes an utterance.** Only `transcribe`
...
- **`v` becomes `b`.** This is Barranquenho betacism. `k` and `q` both yield `[k]`.
