...
- `word` (`str`) — a single word. It is normalised (NFC, lower-cased)
  internally, so case is irrelevant. Pass one token at a time; for a phrase, use
...
- `list[str]` — the phones in reading order, one IPA segment per item.
  `"".join(phonemize(word))` is the word's IPA string. Items may be
...
that belongs to the spec — and is applied automatically by `phonemize` and
`transcribe`.
...
The string-returning adapter other components in the `orthography2ipa`
toolchain expect. `transcribe` phonemises the whole utterance (with sandhi);
`transcribe_word` phonemises a single word (no cross-word effects) and accepts
